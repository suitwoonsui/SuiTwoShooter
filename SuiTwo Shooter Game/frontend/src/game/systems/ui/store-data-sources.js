...
// - token spot prices (/store/catalog) freshness rules
...
(function initStoreDataSources() {
  ...
  /**
   * Terminal-merged store catalog: Provisions items + Stockroom offers + token prices.
  ...
  /**
   * Token USD rates for MEWS/SUI/USDC from shooter GET /store/catalog.
  ...
  async function ensureStoreTokenPrices(options) {
    ...
    try {
      _tokenPricesInFlight = (async () => {
        const catalog = await fetchStoreCatalogRaw({ forceFetch });
        if (!catalog?.success || !catalog.prices) return readTokenPricesFromState(Infinity) || null;
        writeTokenPricesToState(catalog.prices);
