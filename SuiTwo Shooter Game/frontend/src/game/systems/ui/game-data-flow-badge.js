    ...
    badgeDisplay.innerHTML = `
      <div class="menu-badge-info menu-badge-mint-offer badge-clickable" role="button" tabindex="0" title="Mint your Standard badge">
        ...
        <div class="menu-badge-details">
          <p class="menu-badge-games">Games played: ${totalGames}</p>
...
async function createBadgeDisplay(badgeDisplay, badgeData) {
  ...
  const badgeHTML = `
    <div class="menu-badge-info ${hasPendingUpgrade ? 'badge-has-upgrade badge-clickable' : ''}" ${hasPendingUpgrade ? `title="Click to upgrade your badge to ${upgradeData.newTierName}"` : ''}>
      ...
      <div class="menu-badge-details">
        <p class="menu-badge-games">Games Played: …</p>
  ...
  void (async () => {
    ...
    if (typeof window.fetchRegistryGames === 'function' && walletAddress) {
      try {
        ...
        log.warn('FLOW BADGE', 'Error fetching registry games', error);
      }
    }
    const gamesEl = badgeDisplay.querySelector('.menu-badge-games');
    if (gamesEl) {
      gamesEl.textContent = `Games Played: ${totalGames}`;
    }
  })();
}
