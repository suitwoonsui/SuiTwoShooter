// ==========================================
// PLAYER INVENTORY CACHE (reservoir / holdings path)
// ==========================================
// Single source for menu + item-selection UI: populated via GET /api/player/reservoir-bundle/:addr
// TTL 15 minutes; invalidated on purchase; in-game consumes apply optimistically and refetch on game over.
...
const PlayerInventoryCache = {
  ...
  _reservoirInFlightByAddress: new Map(),
  ...
  async fetchReservoirBundleAndCache(address, opts) {
    const addr = String(address || '').trim();
    if (!addr) throw new Error('reservoir-bundle: missing address');
    ...
    const existing = this._reservoirInFlightByAddress.get(addr);
    if (existing) return existing;

    // Split read path: inventory uses /api/inventory. (reservoir-bundle also includes gamePass,
    // but credits/tickets are loaded separately via /api/game-pass.)
    const p = (async () => {
      ...
      this._reservoirInFlightByAddress.delete(addr);
    });

    this._reservoirInFlightByAddress.set(addr, p);
    return p;
  },
  ...
  async runPendingGameOverRefetchIfAny() {
    ...
    try {
      await this.fetchReservoirBundleAndCache(addr);
  ...
  /**
   * Invalidate + reservoir fetch + store inventory UI + game-pass refresh (non-blocking).
  ...
  refreshAfterRewardBackground(address, options) {
    ...
    void (async () => {
      try {
        if (refreshInventory) {
          await this.fetchReservoirBundleAndCache(address);
...
if (typeof window !== 'undefined') {
  ...
  window.prefetchPlayerInventoryReservoir = function prefetchPlayerInventoryReservoir(address) {
    if (!address) return Promise.resolve();
    return PlayerInventoryCache.fetchReservoirBundleAndCache(address).catch((err) => {
      console.warn('[prefetchPlayerInventoryReservoir]', err?.message || err);
    });
  };
}
