...
function renderProjectilePreviews() {
  ...
  // Force Field Preview - cycles through all 3 levels
  ...
  forceFieldCanvasIds.forEach(canvasId => {
    ...
    if (forceFieldCanvas && forceFieldCanvas.offsetParent !== null) {
      ...
      if (ctx) {
        ...
        // Initialize start time for cycling if not set
        if (!forceFieldCanvas.dataset.cycleStartTime) {
          forceFieldCanvas.dataset.cycleStartTime = Date.now().toString();
        }
        
        // Cycle through levels 1, 2, 3 every 2 seconds each (6 second total cycle)
        const cycleStartTime = parseInt(forceFieldCanvas.dataset.cycleStartTime, 10);
        const elapsed = Date.now() - cycleStartTime;
        const levelDuration = 2000; // 2 seconds per level
        const cyclePosition = (elapsed % (levelDuration * 3)) / levelDuration;
        const fieldLevel = Math.floor(cyclePosition) + 1; // 1, 2, or 3
        
        const centerX = forceFieldCanvas.width / 2;
        const centerY = forceFieldCanvas.height / 2;
        
        // Use the shared force field rendering function (matches in-game exactly)
        if (typeof window.renderForceFieldAt === 'function') {
          window.renderForceFieldAt(ctx, centerX, centerY, fieldLevel);
        } else {
          // Fallback if function not loaded yet (shouldn't happen, but safe)
          console.warn('[HOW TO PLAY] renderForceFieldAt not available, force field preview may not render');
        }
        
      // Animate the force field (re-render on next frame for animation and level cycling)
      // Only start animation if canvas is visible
      if (forceFieldCanvas.dataset.animating !== 'true' && forceFieldCanvas.offsetParent !== null) {
        ...
        const animateForceField = (currentTime) => {
          ...
          if (canvas && canvas.offsetParent !== null) {
            // Throttle to ~30fps for animations (saves performance)
            // Also re-render to update level cycling
            if (currentTime - lastFrameTime >= 33) {
              renderProjectilePreviews();
              lastFrameTime = currentTime;
            }
            ...
            // Reset cycle start time when hidden so it restarts when shown again
            delete forceFieldCanvas.dataset.cycleStartTime;
          }
        };
        requestAnimationFrame(animateForceField);
      }
...
function showHowToPlayTab(tabIndex) {
  ...
  if (currentTabContent) {
    ...
    setTimeout(() => {
      showHowToPlayContent(0);
      // Re-render canvas previews after tab becomes visible
      // This ensures canvases in hidden tabs get rendered when their tab is shown
      requestAnimationFrame(() => {
        setTimeout(() => renderProjectilePreviews(), 100);
      });
    }, 50);
...
function showHowToPlayContent(contentIndex) {
  ...
  if (currentSection) {
    ...
    // Re-render canvas previews after section becomes visible
    // This ensures canvases in hidden sections get rendered when their section is shown
    requestAnimationFrame(() => {
      setTimeout(() => renderProjectilePreviews(), 50);
    });
  }
