...
import { getTournamentService, Tournament } from '@/lib/services/tournament/core/tournament-service';
...
const STANDARD_PERCENTAGE = 0.25;    // 25% after creation fee covered (increased from 10% for better creator incentives)
...
export class CreatorRewardService {
  ...
  static async getCreatorRewardStatus(
    ...
    try {
      ...
      PlatformLogger.error('Error getting creator reward status', {
        tournamentObjectId,
  ...
  static async getTournamentsByCreator(
    ...
    try {
      ...
      PlatformLogger.error('Error getting tournaments by creator', {
        creatorAddress,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      return {
  ...
  static async distributeCreatorReward(
    ...
    try {
      ...
      if (creatorRewardUSDCents === 0) {
        PlatformLogger.info('🏆 [CREATOR REWARD] No creator reward to distribute', {
          tournamentId: tournament.tournamentId,
      ...
      PlatformLogger.info('🏆 [CREATOR REWARD] Distributing creator reward', {
        tournamentId: tournament.tournamentId,
        creatorAddress: tournament.createdBy,
        creatorRewardUSDCents,
        creatorRewardUSD,
      ...
      if (!tokenConversion.success || !tokenConversion.tokenAmount) {
        PlatformLogger.error('🏆 [CREATOR REWARD] Failed to convert USD to MEWS', {
          tournamentId: tournament.tournamentId,
          creatorRewardUSD,
      ...
      // Distribute via Sustain (Channel): Regatta → Sustain → Channel
      const adminWallet = getAdminWalletService();
      const adminWalletAddress = adminWallet.getAddress();
      const { buildBatchViaChannel } = await import('@/lib/services/platform/client/platform-client');
      const { signAndSubmitRewardTransactions } = await import('@/lib/services/rewards/executor/rewards-platform-executor');
          ...
          operations: [
            {
              ...
              params: {
                rewards: [
                  {
                    recipientAddress: tournament.createdBy,
                    tokenType: 'MEWS',
                    tokenAmount: mewsAmount,
                  },
                ],
                adminWalletAddress,
                source: 'creator-reward',
              },
            },
          ],
        }
      ...
      if (!buildRes.success || !buildRes.transactions?.length) {
        const err = buildRes.errors?.[0] ?? buildRes.error ?? 'Sustain build failed';
        PlatformLogger.error('🏆 [CREATOR REWARD] Sustain (Channel) build failed', {
          tournamentId: tournament.tournamentId,
          creatorAddress: tournament.createdBy,
          error: err,
        });
      ...
      const creatorAddress = tournament.createdBy ?? '';
      ...
      const digest = execRes.digests[0];
      PlatformLogger.info('🏆 [CREATOR REWARD] Creator reward distributed via Sustain (Channel)', {
        tournamentId: tournament.tournamentId,
        creatorAddress: tournament.createdBy,
        creatorRewardUSDCents,
        creatorRewardUSD,
      ...
      PlatformLogger.error('🏆 [CREATOR REWARD] Error distributing creator reward', {
        tournamentObjectId,
        error: error instanceof Error ? error.message : 'Unknown error',
      });
      return {
