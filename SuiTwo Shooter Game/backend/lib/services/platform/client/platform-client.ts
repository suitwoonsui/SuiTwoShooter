...
// Uses env PLATFORM_BACKEND_URL, ECOSYSTEM_ID, APP_ID, ECOSYSTEM_<id>_API_KEY / API_KEY.
//
...
export function getApiKeyForEcosystem(ecosystemId: string): string | undefined {
  ...
  if (id) {
    const keysToTry = [
      `ECOSYSTEM_${id.toUpperCase().replace(/-/g, '_')}_API_KEY`,
      `ECOSYSTEM_${id.replace(/-/g, '_')}_API_KEY`,
    ];
 ...
 * the platform derives ecosystem/app from the cap. Sends X-API-Key from env (for the game's ecosystem).
 */
export async function callPlatformBackend<T = any>(
  ...
  const apiKey = getApiKeyForEcosystem(getEcosystemIdFromEnv());
...
/**
 * Stats API Client
 * Platform stores/returns player stats (game-defined schema). Pass options.ecosystemId for multi-ecosystem.
 */
...
export const platformGameScoreClient = {
  ...
  async submitScore(
    ...
    const totalGames = ((prev.totalGames as number) ?? 0) + 1;
    const stats: Record<string, unknown> = {
      totalGames,
      totalScore: ((prev.totalScore as number) ?? 0) + params.scoreData.score,
      totalDistance: ((prev.totalDistance as number) ?? 0) + params.scoreData.distance,
      totalCoins: ((prev.totalCoins as number) ?? 0) + params.scoreData.coins,
      totalBossesDefeated: ((prev.totalBossesDefeated as number) ?? 0) + params.scoreData.bossesDefeated,
      totalEnemiesDefeated: ((prev.totalEnemiesDefeated as number) ?? 0) + params.scoreData.enemiesDefeated,
      totalCoinStreak: ((prev.totalCoinStreak as number) ?? 0) + params.scoreData.longestCoinStreak,
      bestScore: Math.max((prev.bestScore as number) ?? 0, params.scoreData.score),
      bestDistance: Math.max((prev.bestDistance as number) ?? 0, params.scoreData.distance),
      bestCoins: Math.max((prev.bestCoins as number) ?? 0, params.scoreData.coins),
      bestBossesDefeated: Math.max((prev.bestBossesDefeated as number) ?? 0, params.scoreData.bossesDefeated),
      bestEnemiesDefeated: Math.max((prev.bestEnemiesDefeated as number) ?? 0, params.scoreData.enemiesDefeated),
      bestCoinStreak: Math.max((prev.bestCoinStreak as number) ?? 0, params.scoreData.longestCoinStreak),
      lastGameDate: Date.now(),
    };
    if (totalGames === 1) stats.firstGameDate = Date.now();
    if (params.playerName != null && params.playerName !== '') stats.playerName = params.playerName;

    const updateRes = await callPlatformBackend<{
      success: boolean;
      transactionBytesBase64?: string;
      address?: string;
      updated?: boolean;
    ...
    if (!updateRes.success) {
      return { success: false, error: updateRes.error ?? 'Hydroscope update failed' };
    }
    if (updateRes.transactionBytesBase64) {
      return { success: true, buildOnly: true, transactionBytesBase64: updateRes.transactionBytesBase64 };
    }
    invalidatePlayerStatsCacheForAddress(params.playerAddress);
    return { success: true, updated: true } as { success: boolean; digest?: string; buildOnly?: boolean; transactionBytesBase64?: string; updated?: boolean; error?: string };
  },
  ...
  /**
   * Trace: Hydroscope participants/claims (Anchor) → regularGameScores.
  ...
  async trace(
    ...
    // Note: This is a best-effort composite trace. Regular scores come from Anchor claims (via Hydroscope).
...
/**
 * Anchor — claims (milestone/achievement completion). Claims are stored on-chain via Anchor.
 * List via Hydroscope; build submit_claim via POST /api/anchor/claims.
 */
export const platformAnchorClient = {
  /**
   * List claims for a participant (Hydroscope reads Anchor). Use for "claimed milestone IDs" by filtering claimType.
  ...
  async listParticipantClaims(
    address: string,
    options?: CallPlatformBackendOptions & { limit?: number; payloadPlayerAddress?: string }
  ): Promise<{
    success: boolean;
    participant?: string;
    claims?: Array<{
 ...
 * When collectionId is set (or derived from ecosystemId for default), only NFTs in that collection count.
 */
export async function getHasSoulboundBadge(
  ...
  const q = new URLSearchParams({ address });
  const collectionId = options?.collectionId ?? getDefaultBadgeCollectionId(options?.ecosystemId);
...
export async function buildShipyardIssueSoulboundMintPermit(
  ...
  const collectionId = params.collectionId ?? getDefaultBadgeCollectionId(getEcosystemIdFromEnv());
  const opts = {
    identityFromCapOnly: true,
    corridorCapabilityObjectId: options?.corridorCapabilityObjectId ?? (getCorridorCapabilityObjectIdFromEnv() || undefined),
    ...options,
