...
export async function loadLeaderboardApiResponse(params: {
  ...
  try {
    const platformRes = await platformStatsClient.getLeaderboard(limit);
    if (platformRes?.success && Array.isArray(platformRes.leaderboard)) {
      PlatformLogger.debug('Leaderboard from platform stats', { count: platformRes.leaderboard.length });
      const leaderboard = platformRes.leaderboard;
