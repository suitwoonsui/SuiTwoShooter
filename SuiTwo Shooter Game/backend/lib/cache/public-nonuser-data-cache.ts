...
export const PUBLIC_LEADERBOARD_BOOTSTRAP_LIMIT = 100;
...
export async function getOrLoadStoreCatalogPayload(options?: {
  ...
  const now = Date.now();
  if (storeCatalogEntry && now - storeCatalogEntry.at < PUBLIC_STORE_CATALOG_TTL_MS) {
  ...
  storeCatalogInFlight = (async () => {
    try {
      const fresh = await fetchCompleteStoreCatalogWithRetry();
