...
export function TournamentCreationFeeEditor({
  ...
  const [feeUsdCents, setFeeUsdCents] = useState(500);
    ...
    try {
      const response = await fetch(getApiUrl('api/game-config/admin'), {
        ...
        body: JSON.stringify({
          ...
          tournamentCreationFeeUsdCents: Math.max(0, Math.round(feeUsdCents)),
  ...
  const liveUsdToSui =
    feeMode === 'usd' && tokenPrices && tokenPrices.SUI > 0
      ? (Math.max(0, feeUsdCents) / 100) / tokenPrices.SUI
      : null;
  const liveUsdToMews =
    feeMode === 'usd' && tokenPrices && tokenPrices.MEWS > 0
      ? (Math.max(0, feeUsdCents) / 100) / tokenPrices.MEWS
      : null;
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '1rem' }}>
            <div>
              ...
              <div style={{ marginTop: '0.5rem', fontSize: '1.2rem', color: styles.buttonPrimary }}>
                {loading ? 'Loading...' : `$${(feeUsdCents / 100).toFixed(2)} (${feeUsdCents} cents)`}
              </div>
            </div>
            <button
              type="button"
