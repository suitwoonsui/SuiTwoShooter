    ...
    <>
      ...
      {migrationMode === 'auto' && (
        <div>
          ...
          <WalletDiscoveryUI
            styles={styles}
            title="🔍 Discover Wallets with Inventory (Old Contract)"
            discoverButtonText="🔍 Discover Wallets with Inventory"
            searchAddress={searchAddress}
            setSearchAddress={setSearchAddress}
            onSearch={() => {
              // Filter discovered wallets if any are found
              if (discoveredWallets.length > 0 && searchAddress.trim()) {
                const found = discoveredWallets.find(w => w.toLowerCase().includes(searchAddress.trim().toLowerCase()));
                if (!found) {
            ...
            onDiscover={handleDiscoverWallets}
            searchingWallet={false}
            discoveringWallets={discoveringWallets}
            discoveredWalletsCount={discoveredWallets.length}
          />
