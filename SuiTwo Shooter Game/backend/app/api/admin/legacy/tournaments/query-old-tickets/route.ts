// @ts-nocheck
// ==========================================
// LEGACY: Admin API - Query Old Contract Tickets
// Reads from old game-pass contract only. See app/api/admin/legacy/README.md.
// ==========================================
...
import { getSonarClient } from '@/lib/services/platform/client/platform-client';
  ...
  async (request: NextRequest) => {
    ...
    if (ticketId !== undefined) {
      ...
      try {
        ...
        const result1 = await readClient.devInspectTransactionBlock({
      ...
      try {
        ...
        const dynamicField = await readClient.getDynamicFieldObject({
          parentId: finalOldGamePassSystemId,
          name: dynamicFieldName,
        });

        if ((dynamicField as { data?: { objectId: string } }).data) {
          const gamePassObjectId = (dynamicField as { data: { objectId: string } }).data.objectId;
          const gamePassObj = await readClient.getObject({
            id: gamePassObjectId,
          ...
          if (gamePassObj.data?.content) {
            ...
            if (tournamentTicketsTable && tournamentTicketsTable.fields && tournamentTicketsTable.fields.id) {
              ...
              const ticketFieldObj = await readClient.getDynamicFieldObject({
                parentId: tableObjectId,
                name: ticketFieldName,
              }) as { data?: { objectId?: string; type?: string; content?: unknown } };
      ...
      const dynamicField = await readClient.getDynamicFieldObject({
        parentId: finalOldGamePassSystemId,
        name: dynamicFieldName,
      }) as { data?: { objectId?: string; content?: unknown; type?: string } };
      ...
      const dynamicFields = await readClient.getDynamicFields({
        parentId: tableObjectId,
      }) as { data: Array<{ name?: unknown }> };

      const dynamicFieldsData = dynamicFields.data;
      console.log(`[QUERY OLD TICKETS] Found ${dynamicFieldsData.length} tickets in old tournament_tickets table`);

      // Query each ticket
      for (const field of dynamicFieldsData) {
