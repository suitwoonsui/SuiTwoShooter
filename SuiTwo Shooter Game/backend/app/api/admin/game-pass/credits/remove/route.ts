// ==========================================
// Game-pass proxy: Remove credits (game admin only)
...
export const POST = withApiHandler(async (request: NextRequest) => {
  ...
  if (!corridorCap?.startsWith('0x')) {
    throw new Error('Remove credits requires Channel. Set CORRIDOR_CAPABILITY_OBJECT_ID (or _TESTNET/_MAINNET) in game backend config/contracts.<network>.json.');
  }
  const build = await buildBatchViaChannel(
    {
      operations: [{
        operationId: 'reservoir-admin-remove-balance',
        params: {
