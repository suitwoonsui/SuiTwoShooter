// ==========================================
// Badge upgrade — step 1: Channel payment to game admin when Helm badge_upgrade_fee > 0.
// ==========================================
...
export const POST = withApiHandler(
  async (request: NextRequest) => {
    ...
    if (feeUsdCents <= 0) {
      PlatformLogger.info('Badge upgrade purchase: no upgrade game fee; skip payment step', { playerAddress });
      return {
        success: true,
    ...
    if (!['SUI', 'MEWS', 'USDC'].includes(paymentToken)) {
      throw new PlatformError(
        ...
        'paymentToken must be SUI, MEWS, or USDC when upgrade fee is configured'
      );
    }
    ...
    if (!built.requiresPayment) {
      throw new PlatformError(
        PlatformErrorCode.TRANSACTION_FAILED,
        'Expected channel-payment transaction when upgrade fee is configured'
      );
    }

    PlatformLogger.info('Badge upgrade purchase: built channel-payment to game admin', {
      playerAddress,
      feeUsdCents,
