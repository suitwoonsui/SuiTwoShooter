...
// Store catalog vs spot prices: use `StoreDataSources` (catalog + /prices/tokens); not keyed here.
// ==========================================
...
/** Per-wallet transaction digest history — bounded (Phase 1). */
const TRANSACTION_DIGEST_MAX_PER_WALLET = 30;
const TRANSACTION_DIGEST_MAX_AGE_MS = 12 * 60 * 60 * 1000;
...
class ApiRequestCache {
  ...
  /**
   * Drop old digests and cap list length per wallet (memory bound).
   * @private
   */
  _pruneTransactionDigestsForWallet(walletAddress) {
    const list = this.transactionDigests.get(walletAddress);
    if (!list || list.length === 0) return;
    const now = Date.now();
    let kept = list.filter((tx) => now - tx.timestamp <= TRANSACTION_DIGEST_MAX_AGE_MS);
    if (kept.length > TRANSACTION_DIGEST_MAX_PER_WALLET) {
      kept.sort((a, b) => a.timestamp - b.timestamp);
      kept = kept.slice(-TRANSACTION_DIGEST_MAX_PER_WALLET);
    }
    if (kept.length === 0) {
      this.transactionDigests.delete(walletAddress);
    } else {
      this.transactionDigests.set(walletAddress, kept);
    }
  }
  ...
  recordTransaction(walletAddress, digest, transactionType) {
    ...
    this._pruneTransactionDigestsForWallet(walletAddress);
