...
function setPrepLoadoutGameItemSelectionFromRow(itemType, levelNum) {
  ...
  const lvl = Number(levelNum || 1) || 1;
  if (!type || !(type in gameItemSelection)) return;

  // Toggle selection per item type (multi-select, max 1 per type).
  if (type === 'destroy_all' || type === 'boss_kill_shot') {
    gameItemSelection[type] = gameItemSelection[type] === true ? false : true;
    return;
  }

  // Multi-level items: clicking same level deselects, otherwise selects that level.
  gameItemSelection[type] = gameItemSelection[type] === lvl ? null : lvl;
}
...
function clearPrepLoadoutGameItemSelectionForType(itemType) {
  const type = String(itemType || '').trim();
  if (!type || !(type in gameItemSelection)) return;
  if (type === 'destroy_all' || type === 'boss_kill_shot') {
    gameItemSelection[type] = false;
  } else {
    gameItemSelection[type] = null;
  }
}

function getCurrentGameItemSelectionSnapshot() {
  return { ...gameItemSelection };
}
...
async function fetchFreshBlockchainInventoryForConsumption(walletAddress) {
  ...
  try {
    if (window.PlayerInventoryCache && typeof window.PlayerInventoryCache.fetchPlayerInventoryAndCache === 'function') {
      await window.PlayerInventoryCache.fetchPlayerInventoryAndCache(walletAddress, { forceRefresh: true }).catch(() => null);
...
async function showItemConsumptionModal(options = {}) {
  ...
  return new Promise(async (resolve) => {
    ...
    // Prefer PlayerInventoryCache from wallet/menu load (15m TTL). No refetch at game start when fresh.
    let inventory = {};
    try {
      const cached = window.PlayerInventoryCache?.getFreshOrNull?.(walletAddress);
      if (cached) {
        inventory = cached;
        log.debug('CONSUMPTION', 'Using cached player inventory (no start refetch)', Object.keys(inventory).length);
        ...
        inventory = inv || {};
        log.debug('CONSUMPTION', 'Loaded inventory via PlayerInventoryCache', Object.keys(inventory).length);
      } else {
        const base = window.GameApi ? window.GameApi.getBaseUrl() : (window.GAME_CONFIG?.GAME_BACKEND_URL || window.GAME_CONFIG?.API_BASE_URL || 'http://localhost:3001/api');
...
if (typeof window !== 'undefined') {
  ...
  window.clearPrepLoadoutGameItemSelectionForType = clearPrepLoadoutGameItemSelectionForType;
  window.getCurrentGameItemSelectionSnapshot = getCurrentGameItemSelectionSnapshot;
}
