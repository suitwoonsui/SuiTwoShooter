...
function buildTournamentDetailsPanelHTML(tournament, playerTicketCount = null) {
  ...
  // Defense-in-depth: hide enter button if endTime has passed even if status is stale.
  const now = Date.now();
  const isEndedByTime = typeof tournament.endTime === 'number' && Number.isFinite(tournament.endTime) && now > tournament.endTime;
  const canEnterTournament = tournament.status === 'active' && !isEndedByTime;
  const walletAddress = getWalletAddressForTournaments();
