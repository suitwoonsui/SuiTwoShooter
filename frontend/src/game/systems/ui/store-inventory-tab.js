...
const StoreInventoryTab = {
  _selected: null,
  _applyPrepRunSelectionHighlights() {
    const root = document.querySelector('#storeInventoryTabContent [data-inventory-items-container]');
    if (!root) return;

    // Clear previous run-selection highlights
    root.querySelectorAll('.inventory-item-level-card').forEach((el) => {
      el.classList.remove('inventory-run-selected');
    });

    const snap =
      typeof window !== 'undefined' && typeof window.getCurrentGameItemSelectionSnapshot === 'function'
        ? window.getCurrentGameItemSelectionSnapshot()
        : null;
    if (!snap || typeof snap !== 'object') return;

    for (const [itemType, levelOrBool] of Object.entries(snap)) {
      if (levelOrBool == null || levelOrBool === false) continue;
      const levelNum = levelOrBool === true ? 1 : Number(levelOrBool || 1) || 1;
      ...
      if (sel) sel.classList.add('inventory-run-selected');
    }
  },
  ...
  async render(walletAddress, paymentToken = 'sui') {
    ...
    try {
      ...
      if (!inventory) {
        // When PlayerInventoryCache is available, use it; else combined menu snapshot from game backend.
        if (window.PlayerInventoryCache?.fetchPlayerInventoryAndCache) {
  ...
  _renderInventoryDisplay(container, inventory) {
    ...
    if (isPrep) {
      this._applyPrepRunSelectionHighlights();
    }
  ...
  _pruneInvalidInventorySelection(itemsByType, isPrep) {
    ...
    if (!itemData || typeof qty !== 'number' || qty < 1) {
      this._selected = null;
      if (isPrep) {
        if (typeof window.clearPrepLoadoutGameItemSelectionForType === 'function') {
          window.clearPrepLoadoutGameItemSelectionForType(itemType);
        } else if (typeof window.clearPrepLoadoutGameItemSelection === 'function') {
  ...
  _groupItemsByType(inventory) {
    const itemsByType = {};
    // Inventory endpoint may include balance-like keys (e.g. credits). Those are not inventory items.
    const excludedTypes = new Set(['credits']);
    ...
    for (const [key, quantity] of Object.entries(inventory)) {
      if (quantity > 0) {
        ...
        if (excludedTypes.has(String(itemType).toLowerCase())) continue;
      ...
      if (btn) {
        ...
        try {
          ...
          if (isConfirmedSuccess) {
            ...
            if (window.PlayerInventoryCache?.fetchPlayerInventoryAndCache) {
              // Force a network refresh so this.render() doesn't read stale cache.
              await window.PlayerInventoryCache.fetchPlayerInventoryAndCache(walletAddress, { forceRefresh: true }).catch(() => {});
            }
