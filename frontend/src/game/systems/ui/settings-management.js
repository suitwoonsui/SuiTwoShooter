...
function loadSettingsToUI() {
  ...
  document.getElementById('soundEffects').addEventListener('change', function() {
    ...
    // Apply immediately so UI toggles affect current audio playback.
    if (typeof applySettings === 'function') {
      applySettings();
    }
    if (typeof saveGameData === 'function') {
  ...
  document.getElementById('backgroundMusic').addEventListener('change', function() {
    ...
    // Apply immediately so the menu music can stop/start right away.
    if (typeof applySettings === 'function') {
      applySettings();
    }
    // Menu decides whether menu music should be playing.
    try {
      if (typeof MenuService !== 'undefined' && typeof MenuService.syncMenuMusicWithSettings === 'function') {
        MenuService.syncMenuMusicWithSettings();
      }
    } catch (_) {}
    if (typeof saveGameData === 'function') {
      saveGameData();
