...
const GameDataFlowService = {
  ...
  async load(walletAddress, options = {}) {
    ...
    if (this.hasActiveLoad(walletAddress)) {
      ...
      // Returning after a game uses `force: true` so stats/badge match the run. If we bail here, the menu
      // footer can refresh (MenuService already ran updateMenuStats) while the badge chip keeps the
      // pre-run games count from the in-flight load's displayBadge.
      if (!options.force) {
        return;
      }
      log.debug('FLOW SERVICE', 'Prior load settled; continuing forced refresh');
    }
    ...
    try {
      ...
      if (badgeData) {
        if (badgeData.success && badgeData.hasBadge && badgeData.badge) {
          ...
          GameDataState.markDataLoaded();
          const tryRenderBadge = async () => {
            if (typeof window.displayBadge === 'function') {
              try {
                await window.displayBadge(badgeData, { forceRefreshStats: options.force === true });
              } catch (err) {
          ...
          // IMPORTANT: `game-data-flow-service.js` can run before `game-data-flow-badge.js` is loaded
          // (see lazy-loader ordering). If we miss the initial render, the main menu badge slot stays empty.
          if (!(await tryRenderBadge())) {
            const maxAttempts = 15;
            let attempts = 0;
            const retryMs = 50;
            const t = setInterval(() => {
              attempts++;
              // Note: do not await inside interval; best-effort render once displayBadge exists.
              if (typeof window.displayBadge === 'function') {
                void tryRenderBadge();
                clearInterval(t);
                return;
              ...
              if (attempts >= maxAttempts) {
                clearInterval(t);
                if (attempts >= maxAttempts && typeof window.displayBadge !== 'function') {
                  console.warn('⚠️ [FLOW SERVICE] displayBadge still unavailable after retries; badge may not render in menu yet');
                }
              }
            }, retryMs);
          }

          // IMPORTANT UX: if there's a pending upgrade, ensure the current badge is rendered first.
          // Then show the upgrade modal (if enabled) after stats/credits/tickets are already settled.
          ...
          if (upgradeHandled) {
            ...
            // Underlying data load is complete; update readiness UI now.
            try {
              if (typeof updateGameReadiness === 'function') {
                updateGameReadiness();
              }
      ...
      if (
        ...
        console.warn('⚠️ [FLOW SERVICE] Auto-opening mint modal after load', {
          addressShort: connectTiming.walletAddressShort,
          autoShowMintModalAfterLoad,
        });
        ...
        setTimeout(() => {
          ...
          // Hard safety gate: never show the mint modal if the backend currently reports the wallet already has a badge.
          // This protects against transient loadBadge/noBadge paths, prefetch failures, or earlier misconfig.
          (async () => {
            try {
              if (walletAddress && window.BadgeService && typeof window.BadgeService.getBadge === 'function') {
                const fresh = await window.BadgeService.getBadge(walletAddress, { includePendingUpgrade: true, bypassCache: true });
                if (fresh && fresh.success === true && fresh.hasBadge === true) {
                  console.warn('⚠️ [FLOW SERVICE] Skipping mint modal: wallet already has a badge', {
                    addressShort: connectTiming.walletAddressShort,
                    badgeId: fresh.badge && fresh.badge.badgeId ? String(fresh.badge.badgeId).slice(0, 12) + '…' : null,
                    tier: fresh.badge && typeof fresh.badge.tier === 'number' ? fresh.badge.tier : null,
                  });
                  return;
                }
              }
            } catch (e) {
              console.warn('⚠️ [FLOW SERVICE] Mint modal guard badge check failed (non-fatal); proceeding', e);
            }
      ...
      if (typeof window.prefetchPlayerInventory === 'function' && walletAddress) {
        window.prefetchPlayerInventory(walletAddress);
      }
