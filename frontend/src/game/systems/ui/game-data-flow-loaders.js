...
async function loadBadge(address) {
  ...
  if (prefetched && prefetched.address === address && (Date.now() - prefetched.at) < BADGE_PREFETCH_TTL_MS && prefetched.data) {
    const d = prefetched.data;
    const ok = d && typeof d === 'object' && d.success === true;
    const has = d && typeof d === 'object' && d.hasBadge === true;
    // IMPORTANT: never let a transient prefetch failure force the UI into "no badge → mint" flow.
    // Only trust prefetched results when they're successful, or explicitly hasBadge:true.
    if (ok || has) {
      log.debug('FLOW LOADERS', 'Using prefetched badge', {
        success: Boolean(ok),
        hasBadge: Boolean(has),
        ageMs: Date.now() - prefetched.at,
      });
      return d;
    }
    log.debug('FLOW LOADERS', 'Ignoring failed prefetched badge; fetching fresh', {
      success: d && typeof d === 'object' ? d.success : undefined,
      hasBadge: d && typeof d === 'object' ? d.hasBadge : undefined,
      ageMs: Date.now() - prefetched.at,
    });
  }
 ...
 * Canonical implementation lives in `StatsService` (loaded early in MENU_SCRIPTS).
 * @deprecated Prefer `window.StatsService.fetchRegistryGames`.
 */
async function fetchRegistryGames(walletAddress, opts = {}) {
  if (window.StatsService && typeof window.StatsService.fetchRegistryGames === 'function') {
    return await window.StatsService.fetchRegistryGames(walletAddress, opts);
  }
  // Should not happen if lazy-loader ordering is intact, but keep a safe default.
  log.warn('FLOW LOADERS', 'StatsService.fetchRegistryGames not available; returning 0');
  return 0;
}

/**
 * Warm game-backend caches for Hydroscope stats + reservoir (same keys as GET /api/stats/:addr and /api/reservoir/:addr).
...
if (typeof window !== 'undefined') {
  ...
  // Keep legacy global but ensure StatsService remains the source of truth.
  // If StatsService was loaded, it already set window.fetchRegistryGames; do not override.
  if (!window.StatsService?.fetchRegistryGames && !window.fetchRegistryGames) {
