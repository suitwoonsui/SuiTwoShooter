...
async function ensureStoreCatalogLoaded() {
  ...
  if (hasRows) {
    ...
    if (typeof window !== 'undefined' && window.StoreDataSources && window.StoreDataSources.ensureStoreTokenPrices) {
      try {
        await window.StoreDataSources.ensureStoreTokenPrices({ forceFetch: true });
      } catch (_) {
        /* non-fatal */
      }
    }
...
async function loadStoreItems() {
  ...
  try {
    ...
    if (typeof window !== 'undefined' && window.StoreDataSources && window.StoreDataSources.ensureStoreTokenPrices) {
      try {
        const spot = await window.StoreDataSources.ensureStoreTokenPrices({ forceFetch: true });
        if (!spot && state) {
          state.tokenPrices = null;
          state.tokenPricesTimestamp = timestamp;
        }
        if (!spot && typeof StoreService !== 'undefined' && StoreService._state) {
          StoreService._state.tokenPrices = null;
          StoreService._state.tokenPricesTimestamp = timestamp;
        }
        if (!spot) {
          console.warn('⚠️ [STORE ITEM LOADER] Token spot prices unavailable');
        }
      } catch (e) {
        console.warn('⚠️ [STORE ITEM LOADER] ensureStoreTokenPrices failed', e);
        if (state) {
          state.tokenPrices = null;
          state.tokenPricesTimestamp = timestamp;
        }
