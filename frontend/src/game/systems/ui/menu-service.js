...
const MenuService = {
  ...
  /**
   * Ensure menu music matches `gameSettings.backgroundMusic`.
   * Single source of truth for starting menu music.
   */
  syncMenuMusicWithSettings() {
    try {
      const enabled = typeof gameSettings !== 'undefined' ? Boolean(gameSettings.backgroundMusic) : true;
      if (!enabled) {
  ...
  async show(options = {}) {
    ...
    // Start/stop menu music based on settings (single source of truth).
    this.syncMenuMusicWithSettings();
    ...
    if (this._gameDataFlow) {
      if (window.walletAPIInstance?.isConnected()) {
        ...
        // Ensure the main-menu badge chip is visible when wallet is connected.
        // The badge can be rendered in other contexts (e.g. Store) but the menu slot is separate (#menuBadgeDisplay).
        try {
          console.log('🧩 [MENU] Ensuring #menuBadgeDisplay is rendered', {
            addressPrefix: address ? String(address).slice(0, 10) : null,
            hasGameDataBadge: typeof GameDataState !== 'undefined' ? Boolean(GameDataState.badge) : null,
            badgeDisplayEl: Boolean(document.getElementById('menuBadgeDisplay')),
          });
          ...
          if (b && b.success === true && b.hasBadge === true && typeof window.displayBadge === 'function') {
            // Fast path: reuse cached badge and render immediately.
            void window.displayBadge(b).catch(() => {});
          } else if (address && typeof window.loadMenuBadgeDisplay === 'function') {
            // Fallback: lightweight load (flow handles dedupe; skipBalance avoids slow RPC).
            void window.loadMenuBadgeDisplay(address);
          } else if (typeof window.showBadgeDisplay === 'function') {
            // Last resort: show any already-rendered HTML.
