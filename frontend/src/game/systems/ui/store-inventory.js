 ...
 * @param {Record<string, number>|null|undefined} preloadedInventory - When set (e.g. from PlayerInventoryCache), skip fetch
 */
async function loadInventoryDisplay(preloadedInventory = null) {
  ...
  try {
    if (window.PlayerInventoryCache) {
      ...
      console.log('📦 [STORE INVENTORY] Loaded via player inventory cache');
      return;
    }
  } catch (e) {
    console.warn('📦 [STORE INVENTORY] Inventory bundle prefetch failed, falling back to /inventory', e);
  }
...
function updateItemCardsInventory(inventory) {
  ...
  cards.forEach(card => {
    ...
    // Balances like credits are not inventory items and shouldn't show as "Owned".
    if (String(itemId || '').toLowerCase() === 'credits') return;
