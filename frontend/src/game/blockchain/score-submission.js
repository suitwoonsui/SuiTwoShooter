...
async function submitScoreToBlockchain(gameStats, playerName = '') {
  ...
  // Prefer the snapshot captured at game over (survives resets/cleanup before the user clicks Save/Skip).
  const replay =
    gameStats && typeof gameStats === 'object' && gameStats.replay && Array.isArray(gameStats.replay.events)
      ? gameStats.replay
      : (typeof window !== 'undefined' &&
          window.ReplayRecorder &&
          typeof window.ReplayRecorder.getReplay === 'function')
      ...
      if (result.badge) {
        if (result.badge.canMint) {
          ...
          setTimeout(() => {
            (async () => {
              try {
                // Safety check: never prompt to mint if the wallet already has a badge.
                // This prevents false positives when backend badge detection lags or was previously misconfigured.
                const addr =
                  (window.walletAPIInstance && window.walletAPIInstance.isConnected() && window.walletAPIInstance.getAddress())
                    ...
                    : null;
                if (addr && window.BadgeService && typeof window.BadgeService.getBadge === 'function') {
                  const fresh = await window.BadgeService.getBadge(addr, { includePendingUpgrade: true, bypassCache: true });
                  if (fresh && fresh.success === true && fresh.hasBadge === true) {
                    console.warn('⚠️ [BADGE] Skipping mint modal: wallet already has a badge', {
                      address: addr,
                      badgeId: fresh.badge && fresh.badge.badgeId ? fresh.badge.badgeId : undefined,
                      tier: fresh.badge && typeof fresh.badge.tier === 'number' ? fresh.badge.tier : undefined,
                    });
                    return;
                  }
                }
              } catch (e) {
                console.warn('⚠️ [BADGE] Mint modal guard check failed (non-fatal); proceeding with modal', e);
              }
          ...
          setTimeout(async () => {
            if (window.BadgeUI && window.BadgeUI.showTierUpgradeModal) {
              try {
                ...
                const currentTier = badgeData.badge.tier;

                // Guard against bogus "upgrade to Standard" prompts (e.g. newTier=0, or already at/above newTier).
                if (typeof newTier !== 'number' || newTier <= 0) {
                  console.warn('⚠️ [BADGE] Ignoring tierUpgraded with non-upgrade tier', { newTier, currentTier });
                  return;
                }
                if (typeof currentTier === 'number' && currentTier >= newTier) {
                  console.warn('⚠️ [BADGE] Ignoring tierUpgraded because badge is already at/above newTier', {
                    newTier,
                    currentTier,
                  });
                  return;
                ...
                const oldTier = Math.max(0, newTier - 1);
