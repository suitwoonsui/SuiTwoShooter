...
async function getBadge(playerAddress = null, options = {}) {
  ...
  if (!bypassCache && includePendingUpgrade && badgePuCache.address === address && badgePuCache.data && (now - badgePuCache.timestamp) < badgePuCache.ttlMs) {
    console.log('📋 [BADGE] Using in-memory PU badge cache for address:', address, {
      ageMs: now - badgePuCache.timestamp,
      ttlMs: badgePuCache.ttlMs,
      cachedAt: badgePuCache.timestamp,
      now,
    });
  ...
  const requestPromise = (async () => {
    ...
    console.log('🌐 [BADGE] Fetching badge from backend', {
      address,
      includePendingUpgrade,
      bypassCache,
      url: `${API_BASE_URL}/badges/${address}${qs}`,
    });
    ...
    try {
      const summary = data && typeof data === 'object'
        ? {
            ...
            badgeId: data.badge && data.badge.badgeId ? String(data.badge.badgeId).slice(0, 12) + '…' : null,
            tier: data.badge && typeof data.badge.tier === 'number' ? data.badge.tier : null,
            hasPendingUpgrade: data.pendingUpgrade && typeof data.pendingUpgrade === 'object'
              ? Boolean(data.pendingUpgrade.hasPendingUpgrade)
              : null,
          }
        : { success: null, hasBadge: null, badgeId: null, tier: null, hasPendingUpgrade: null };
      console.log('✅ [BADGE] Badge fetch completed', { address, includePendingUpgrade, summary });
    } catch (_) {}

    // Only cache successful responses.
    // Otherwise, a transient backend failure can get "stuck" in memory and keep returning hasBadge:false.
    if (data && typeof data === 'object' && data.success === true) {
      ...
      if (includePendingUpgrade) {
        badgePuCache = { address, data, timestamp: now, ttlMs: badgePuCache.ttlMs };
      }
    } else if (includePendingUpgrade && badgePuCache.address === address) {
      // Do not keep a failed includePendingUpgrade response around.
...
if (typeof window !== 'undefined') {
  window.BadgeService = {
    ...
    getTierName,
    clearBadgeCache,
    /** Debug helper: inspect badge caches without fetching. */
    __debugCacheState: function () {
      return {
        badgeCache: {
          address: badgeCache.address,
          timestamp: badgeCache.timestamp,
          ageMs: badgeCache.timestamp ? (Date.now() - badgeCache.timestamp) : null,
          cacheDuration: badgeCache.cacheDuration,
          hasData: Boolean(badgeCache.data),
        },
        badgePuCache: {
          address: badgePuCache.address,
          timestamp: badgePuCache.timestamp,
          ageMs: badgePuCache.timestamp ? (Date.now() - badgePuCache.timestamp) : null,
          ttlMs: badgePuCache.ttlMs,
          hasData: Boolean(badgePuCache.data),
        },
        inFlightKeys: Array.from(badgeInFlightByAddress.keys()),
      };
    },
  };
}
