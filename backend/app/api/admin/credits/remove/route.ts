// ==========================================
// Admin API - Remove Credits from Player's Game Pass
// ==========================================
...
export const POST = withApiHandler(
  async (request: NextRequest) => {
    ...
    const result = await gamePassService.adminRemoveCredits(playerAddress, amount);

    if (!result.success) {
      throw new Error(result.error || 'Failed to remove credits');
    }
    ...
    return {
      ...
      message: `Successfully removed ${amount} credit${amount !== 1 ? 's' : ''} from player`,
    };
  }
);
