// ==========================================
// Admin API - Remove Items from Inventory
// ==========================================
...
export const POST = withApiHandler(
  async (request: NextRequest) => {
    ...
    BadgeLogger.info('Server-side request to remove items', {
      playerAddress,
      items,
    });

    // Use consumeItems which removes items (admin can use it)
    ...
    BadgeLogger.info('Items removed successfully', {
      playerAddress,
    ...
    return {
      success: true,
      digest: result.digest,
      message: `Successfully removed ${items.length} item(s) from inventory`,
    };
  }
);
