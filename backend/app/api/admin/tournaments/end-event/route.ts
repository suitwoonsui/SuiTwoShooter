// ==========================================
// Admin API: End Tournament (Tide lifecycle callback)
// ==========================================
...
export const POST = withApiHandler(async (request: NextRequest) => {
  ...
  if (transactions.length === 0) {
    ...
    if (!senderAddress?.startsWith('0x')) {
      throw new Error('Game admin wallet address is required to build end-event tx (cap holder signs and pays gas).');
    }
    ...
    if (!buildRes.success || !buildRes.transactions?.length) {
      throw new Error(`Failed to build end-event tx: ${buildRes.errors?.join('; ') ?? buildRes.error ?? 'Unknown error'}`);
    }
    transactions = buildRes.transactions.map((bytesBase64) => ({ bytesBase64 }));
    PlatformLogger.info('🧾 [END-EVENT] Built via platform channel (game corridor admin cap), signing and submitting', {
      eventObjectId,
    });
  } else {
    PlatformLogger.info('🧾 [END-EVENT] Tide lifecycle callback: sign and submit', {
      eventObjectId,
  ...
  if (errors.length > 0) {
    PlatformLogger.error('🧾 [END-EVENT] Some transactions failed', { eventObjectId, errors });
  ...
  return {
    ...
    eventObjectId,
    digests,
    message: `Signed and submitted ${digests.length} end-event transaction(s).`,
  };
});
