...
export const POST = withApiHandler(async (request: NextRequest) => {
  ...
  if (!corridorAdminCap?.startsWith('0x')) {
    throw new Error('Add tickets requires CorridorAdminCap. Set CORRIDOR_ADMIN_CAP_OBJECT_ID (or _TESTNET/_MAINNET) in game backend config/contracts.<network>.json.');
  }
