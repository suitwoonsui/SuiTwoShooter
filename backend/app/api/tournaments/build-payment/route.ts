...
export const POST = withApiHandler(
  async (request: NextRequest) => {
    ...
    if (gameAdminTournamentCreationFeeUSDCents == null) {
      // Treat as free (0) by default so local/dev + freshly deployed games can still create tournaments.
      // If you want to enforce a business fee, configure it via platform app config / Helm.
      gameAdminTournamentCreationFeeUSDCents = 0;
      creationFeeModeUsed = 'usd';
      PlatformLogger.warn('🏆 [BUILD PAYMENT] Tournament creation fee not configured; defaulting to 0', {
        playerAddress,
        paymentToken,
      });
    }
