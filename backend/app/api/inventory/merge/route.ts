...
import { buildBatchViaChannel, buildPlatformCallOptions } from '@/lib/services/platform/client/platform-client';
...
function deriveReservoirRecipeId(params: { itemType: number; sourceLevel: number; targetLevel: number; isHyperMerge: boolean }): string {
  // Reservoir recipe IDs are app-defined bytes; we use a stable string convention.
  // If your on-chain recipe IDs differ, pass `recipeId` from the caller to override.
...
export const POST = withApiHandler(
  async (request: NextRequest) => {
    const body = await getRequestBody<{
      ...
      /** Optional override for Reservoir recipeId (string or bytes array). */
      recipeId?: string | number[];
      /** Optional multiplier (number of merges); defaults to 1. */
      multiplier?: number;
      ecosystemId?: string;
    }>(request);
    ...
    const corridorCapabilityObjectId = (platformOptions.corridorCapabilityObjectId ?? '').trim();
    if (!corridorCapabilityObjectId) {
      throw new PlatformError(
        PlatformErrorCode.INVALID_ADDRESS,
        'corridorCapabilityObjectId (CorridorCap) is required (send X-Corridor-Capability-Object-Id or configure it in backend config)'
      );
    }
    ...
    const recipeId =
      body.recipeId != null
        ? body.recipeId
        : deriveReservoirRecipeId({ itemType: contractItemType, sourceLevel, targetLevel, isHyperMerge });
    const multiplier = typeof body.multiplier === 'number' && body.multiplier >= 1 && Number.isInteger(body.multiplier) ? body.multiplier : 1;
