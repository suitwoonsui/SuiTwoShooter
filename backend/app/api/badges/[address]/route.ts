...
export const GET = withApiHandler(
  async (
    ...
    let hasBadge = platformResult.hasBadge ?? false;
    ...
    // Parallel: game config, Sonar tier (reuse Shipyard badgeId), and Hydroscope totalGames for gamesPlayed + upgrade hint.
    // getBadge() does not populate gamesPlayed on-chain (Shipyard row uses 0); Hydroscope is the source of truth (same as GET /stats).
    ...
    const [gameConfigRes, chainBadge, hydroscopeStats] = await Promise.all([
      ...
      platformGameScoreClient.getPlayerStats(playerAddress, statsOptions).catch((err) => {
        PlatformLogger.warn('Hydroscope stats fetch failed for badge response', {
          playerAddress,
    ...
    const hydroTotalGames =
      hydroscopeStats && hydroscopeStats.success === true && typeof hydroscopeStats.totalGames === 'number'
        ? hydroscopeStats.totalGames
        : undefined;
    const gamesPlayedForBody = hydroTotalGames ?? chainBadge?.gamesPlayed ?? 0;

    const badgeId = (platformResult.badgeId || chainBadge?.badgeId || '').trim();
    // If the chain badge row isn't available yet (indexing delay, misconfigured registry, etc),
    // we still treat the Shipyard soulbound badge as authoritative for "hasBadge".
    // Tier falls back to 0 (Standard) and timestamps default to 0.
    const resolvedTier =
      chainBadge && Number.isFinite(chainBadge.tier) ? chainBadge.tier : 0;
    ...
    if (!includePu) {
      return {
        ...
        badge: {
          badgeId,
          tier,
          gamesPlayed: gamesPlayedForBody,
    ...
    if (!chainBadge?.badgeId) {
      PlatformLogger.warn('Shipyard badge exists but chain badge row not resolved; returning tier=0 fallback', {
        playerAddress,
        badgeId,
        gamesPlayedForBody,
      });
    ...
    const upgrade = await badgeService.checkBadgeUpgrade(playerAddress, {
      ...
      ...(hydroTotalGames !== undefined ? { totalGames: hydroTotalGames } : {}),
    });
