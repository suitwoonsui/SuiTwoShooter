...
export function CreditManagement({
  ...
  const handleLoadPlayers = async () => {
    ...
    try {
      const response = await fetch(getApiUrl('api/admin/credits/list-players'));
  ...
  return (
    <div style={{ padding: '1.5rem', backgroundColor: styles.bgSecondary, borderRadius: '8px', border: `1px solid ${styles.border}` }}>
      ...
      <div style={{ padding: '1rem', backgroundColor: styles.bgTertiary, borderRadius: '4px', border: `1px solid ${styles.border}`, marginBottom: '2rem' }}>
        <h3 style={{ marginTop: 0, marginBottom: '1rem', color: styles.text }}>🔍 Discover Wallets with Game Passes</h3>
        <button
          type="button"
          ...
          style={{
            ...
            backgroundColor: discoveringWallets || !isAdminWalletConnected ? styles.buttonDisabled : styles.buttonPrimary,
            color: 'white',
            border: 'none',
            borderRadius: '4px',
        ...
        {discoveredWallets.length > 0 && (
          <div style={{ marginTop: '1rem' }}>
            ...
            >
              {discoveredWallets.map((address, index) => (
                <div
                  key={index}
                  style={{
                    padding: '0.5rem',
                  ...
                  title="Click to copy address and fill in player address field"
                >
                  {address}
                </div>
              ))}
              ...
              <tbody>
                {playersList.map((player) => (
                  <tr key={player.address} style={{ borderBottom: `1px solid ${styles.border}` }}>
                    <td style={{ padding: '0.5rem', fontFamily: 'monospace', fontSize: '0.85rem', color: styles.text }}>
                      {player.address.substring(0, 10)}...{player.address.substring(player.address.length - 8)}
                    </td>
            ...
            <div style={{ marginTop: '1rem', padding: '0.75rem', borderRadius: '4px', backgroundColor: addCreditResult.success ? styles.bgSuccess : styles.bgError, border: `1px solid ${addCreditResult.success ? styles.borderSuccess : styles.borderError}` }}>
              {addCreditResult.success ? (
                <div style={{ color: styles.text }}>
                  ✅ Credits added successfully! New total: {addCreditResult.gamesRemaining} {addCreditResult.digest && `| Transaction: ${addCreditResult.digest.substring(0, 20)}...`}
                </div>
              ) : (
          ...
          {removeCreditResult && (
            <div style={{ marginTop: '1rem', padding: '0.75rem', borderRadius: '4px', backgroundColor: removeCreditResult.success ? styles.bgSuccess : styles.bgError, border: `1px solid ${removeCreditResult.success ? styles.borderSuccess : styles.borderError}` }}>
              {removeCreditResult.success ? (
                <div style={{ color: styles.text }}>
                  ✅ Credits removed successfully! New total: {removeCreditResult.gamesRemaining} {removeCreditResult.digest && `| Transaction: ${removeCreditResult.digest.substring(0, 20)}...`}
                </div>
              ) : (
          ...
          {setCreditResult && (
            <div style={{ marginTop: '1rem', padding: '0.75rem', borderRadius: '4px', backgroundColor: setCreditResult.success ? styles.bgSuccess : styles.bgError, border: `1px solid ${setCreditResult.success ? styles.borderSuccess : styles.borderError}` }}>
              {setCreditResult.success ? (
                <div style={{ color: styles.text }}>
                  ✅ Credits set successfully! New total: {setCreditResult.gamesRemaining} {setCreditResult.digest && `| Transaction: ${setCreditResult.digest.substring(0, 20)}...`}
                </div>
              ) : (
