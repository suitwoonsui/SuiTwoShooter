  ...
  const handleDiscoverWallets = async () => {
    ...
    if (result.success && result.wallets) {
      ...
      if (result.wallets.length === 0) {
        alert('No wallets with badges found.');
      }
    } else {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
      ...
      <div style={{ padding: '1rem', backgroundColor: styles.bgSecondary, borderRadius: '4px', border: `1px solid ${styles.border}` }}>
        <h3 style={{ marginTop: 0, marginBottom: '1rem', color: styles.text }}>🔍 Discover Wallets with Badges</h3>
        <button
          type="button"
        ...
        >
          {discoveringWallets ? 'Discovering...' : '🔍 Discover All Wallets with Badges'}
        </button>
        {discoveredWallets.length > 0 && (
          <div style={{ marginTop: '1rem' }}>
            <p style={{ marginBottom: '0.5rem', color: styles.textSecondary }}>
              Found {discoveredWallets.length} wallet(s) with badges:
            </p>
            <div
              style={{
                maxHeight: '300px',
