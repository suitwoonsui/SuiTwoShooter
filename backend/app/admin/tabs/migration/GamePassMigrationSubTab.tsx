    ...
    <>
      ...
      {gamePassMigrationMode === 'auto' && (
        <div>
          ...
          <WalletDiscoveryUI
            styles={styles}
            title="🔍 Discover Wallets with Game Passes (Old Contract)"
            discoverButtonText="🔍 Discover Wallets with Game Passes"
            searchAddress={searchAddress}
            setSearchAddress={setSearchAddress}
            onSearch={() => {
              // Filter discovered wallets if any are found
              if (discoveredGamePassWallets.length > 0 && searchAddress.trim()) {
                const found = discoveredGamePassWallets.find(w => w.toLowerCase().includes(searchAddress.trim().toLowerCase()));
                if (!found) {
            ...
            onDiscover={handleDiscoverGamePassWallets}
            searchingWallet={false}
            discoveringWallets={discoveringGamePassWallets}
            discoveredWalletsCount={discoveredGamePassWallets.length}
          />
          ...
          {discoveredGamePassWallets.length > 0 && (
            <div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: styles.bgTertiary, borderRadius: '4px', border: `1px solid ${styles.border}` }}>
              ...
              <div style={{ marginTop: '0.5rem', maxHeight: '200px', overflowY: 'auto', fontSize: '0.9rem', color: styles.textSecondary }}>
                {(searchAddress.trim() 
                  ? discoveredGamePassWallets.filter(w => w.toLowerCase().includes(searchAddress.trim().toLowerCase()))
                  : discoveredGamePassWallets
                ).map((wallet, index) => (
