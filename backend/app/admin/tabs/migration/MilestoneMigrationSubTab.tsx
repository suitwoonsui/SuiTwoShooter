      ...
      <div style={{ padding: '1.5rem', backgroundColor: styles.bgSecondary, border: `1px solid ${styles.border}`, borderRadius: '8px' }}>
        ...
        {claimsMigrationMode === 'auto' && (
          <div style={{ marginBottom: '1rem' }}>
            ...
            <WalletDiscoveryUI
              styles={styles}
              title="🔍 Discover Wallets with Milestone Claims (Old Contract)"
              discoverButtonText="🔍 Discover Wallets with Milestone Claims"
              searchAddress={searchAddress}
              setSearchAddress={setSearchAddress}
              onSearch={() => {
                // Filter discovered wallets if any are found
                if (discoveredMilestoneWallets.length > 0 && searchAddress.trim()) {
                  const found = discoveredMilestoneWallets.find(w => w.toLowerCase().includes(searchAddress.trim().toLowerCase()));
                  if (!found) {
              ...
              onDiscover={handleDiscoverMilestoneWallets}
              searchingWallet={false}
              discoveringWallets={discoveringMilestoneWallets}
              discoveredWalletsCount={discoveredMilestoneWallets.length}
            />
            ...
            {discoveredMilestoneWallets.length > 0 && (
              <div style={{ marginTop: '1rem', padding: '1rem', backgroundColor: styles.bgTertiary, borderRadius: '4px', border: `1px solid ${styles.border}` }}>
                ...
                <div style={{ marginTop: '0.5rem', maxHeight: '200px', overflowY: 'auto', fontSize: '0.9rem', color: styles.textSecondary }}>
                  {(searchAddress.trim() 
                    ? discoveredMilestoneWallets.filter(w => w.toLowerCase().includes(searchAddress.trim().toLowerCase()))
                    : discoveredMilestoneWallets
                  ).map((wallet, index) => (
