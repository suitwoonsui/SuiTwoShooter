...
export async function register() {
  ...
  if (process.env.NEXT_RUNTIME === 'nodejs' && process.env.LEADERBOARD_PUBLIC_CACHE_WARM_ENABLED !== 'false') {
    const limit = Math.max(1, Math.min(parseInt(process.env.LEADERBOARD_PUBLIC_CACHE_WARM_LIMIT || '200', 10) || 200, 200));
    // Match the cadence to the cache TTL used by bootstrap/public routes.
  ...
  if (process.env.NODE_ENV === 'development' && process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    setTimeout(() => {
      // Keep warmups via HTTP API calls (Platform + game backend are separate apps/environments).
      // Consolidate warmups into a single fan-out route to reduce request spam.
      fetch(
        `${base}/api/warmup?source=server_warmup&dummyAddress=${encodeURIComponent(
          WARMUP_DUMMY_ADDRESS
        )}&limit=10&platform=1`
      ).catch(() => {});
    }, 5000);
  }
