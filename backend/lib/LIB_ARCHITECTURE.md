...
| **platformStatsClient** | Hydroscope API over **Wake**: on-chain **stats** (metrics) + **marks** (claim-like / one-shot keys; same Wake store, different key semantics per `wake.move`) | `api/hydroscope/*` |
| **platformAnchorClient** / **platformMilestonesClient** | Anchor claim **attestations** (sessions/milestones), Aquifer definitions | claims, definitions |
...
**Platform module map (see docs/results.md):** Aquifer (definitions), Provisions (catalog), Anchor (claim/session **attestation** — verifiable claims, not Wake’s u64 footprint rows), Insignia (per-wallet progression KV per app), Rain + Sustain (rewards), Reservoir (consumables repository—e.g. game-pass play credits consumed via platform APIs, not user wallet coin balances), Shipyard (badges/NFTs), Logbook (audit), Hydroscope (API over **Wake**: **stats** + **marks** in one on-chain footprint store; plus Anchor-derived traces where Hydroscope exposes them), Channel (batch + execute), Station/Regatta (events/tournaments), Helm (app config), Gauge (prices), Glacier (vaults), Terminal (store).
