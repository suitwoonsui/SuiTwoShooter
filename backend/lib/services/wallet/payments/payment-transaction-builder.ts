...
// Shared service for building payment transactions (SUI/MEWS/USDC); balance check + gas.
// I: getConfig() (network, token type IDs); balance-checker (checkPlayerTokenBalanceForPurchase); SuiClient (passed in). Store/tournament payments often go via platform Channel; this builder is for game-built payment tx where needed.
// ==========================================

import { SuiClient, getFullnodeUrl } from '@mysten/sui/client';
import { Transaction } from '@mysten/sui/transactions';
import { getConfig } from '@/config/config';
import { getSonarClient, platformSonarClient } from '@/lib/services/platform/client/platform-client';
import { checkPlayerTokenBalanceForPurchase } from '@/lib/services/wallet/balance/balance-checker';
...
export class PaymentTransactionBuilder {
  ...
  private getPaymentTokenInfo(paymentToken: PaymentToken): {
    ...
    if (paymentToken === 'SUI') {
      ...
      PlatformLogger.debug('Payment token info for MEWS', {
        targetNetwork,
  ...
  private async preparePaymentCoin(
    ...
    if (paymentToken === 'SUI') {
      ...
      try {
        const coins = (await getSonarClient().getCoins({
          owner: playerAddress,
          coinType: tokenInfo.tokenType,
        })) as { data?: Array<{ coinObjectId: string; balance: string }> };
  ...
  private async checkBalance(paymentConfig: PaymentConfig): Promise<{
    ...
    PlatformLogger.debug('Payment balance check', {
    ...
    if (paymentConfig.paymentToken === 'MEWS') {
      PlatformLogger.info('MEWS Token Type ID Check', {
        network: this.config.sui.network,
    ...
    try {
      ...
      PlatformLogger.error('Payment balance check failed', {
        network: this.config.sui.network,
        paymentToken: paymentConfig.paymentToken,
        tokenType: tokenInfo.tokenType,
        walletAddress: paymentConfig.playerAddress,
  ...
  async buildPaymentTransaction(
    ...
    try {
      ...
      // Build transaction via platform (don't sign - frontend will sign)
      ...
      const built = await platformSonarClient.buildTransaction({
        ...
        sender: paymentConfig.playerAddress,
        gasBudget: finalGasBudget,
      });

      return {
        success: true,
        transaction: built.transactionBytesBase64,
