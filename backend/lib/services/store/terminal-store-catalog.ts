...
export type StoreCatalogPayload = {
  ...
  offerOrder: string[];
  /** SHA-256 hex from Aqueduct Terminal (`terminalCatalogHashV1`); absent on older platform deploys. */
...
/** Keep ordering aligned with Aqueduct `buildTerminalOfferOrderFallback` (terminal-offer-order-fallback.ts). */
...
function isCatalogContentHashPresent(h: unknown): h is string {
...
export function shouldAcceptAsNewSnapshot(next: StoreCatalogPayload, prev: StoreCatalogPayload | null): boolean {
  ...
  const nhRaw = next.catalogContentHash;
  const phRaw = prev.catalogContentHash;
  if (isCatalogContentHashPresent(nhRaw) && isCatalogContentHashPresent(phRaw)) {
    const nh = nhRaw.trim().toLowerCase();
    const ph = phRaw.trim().toLowerCase();
    PlatformLogger.debug('[STORE CATALOG] snapshot guard: catalogContentHash path', {
      prevSlice: ph.slice(0, 12),
      nextSlice: nh.slice(0, 12),
      changed: nh !== ph,
    });
    return true;
  }
...
async function fetchFreshStoreCatalog(): Promise<StoreCatalogPayload> {
  ...
  const rawHash = (terminalRes as { catalogContentHash?: unknown }).catalogContentHash;
  const catalogContentHash =
    typeof rawHash === 'string' && /^[a-f0-9]{64}$/i.test(rawHash.trim()) ? rawHash.trim().toLowerCase() : undefined;
  ...
  PlatformLogger.info('[STORE CATALOG] Fresh payload fetched', {
    ...
    terminalDefinitionCount: terminalRes?.definitions && typeof terminalRes.definitions === 'object'
      ...
      : undefined,
    catalogContentHashSlice: catalogContentHash ? catalogContentHash.slice(0, 12) : null,
  });
  return payload;
}
