...
export class BadgeImageCache {
  ...
  constructor(
    ...
    BadgeLogger.debug('BadgeImageCache initialized', { ttl, maxSize });
  }
  ...
  get(tier: number): Uint8Array | null {
    ...
    if (!cached) {
      BadgeLogger.debug(`Cache miss for tier ${tier}`);
      return null;
    }
    ...
    if (age > this.ttl) {
      BadgeLogger.debug(`Cache expired for tier ${tier} (age: ${age}ms)`);
      this.cache.delete(cacheKey);
      return null;
    }
    
    BadgeLogger.debug(`Cache hit for tier ${tier} (age: ${age}ms)`);
    return cached.data;
  }
  ...
  set(tier: number, data: Uint8Array): void {
    ...
    BadgeLogger.debug(`Cached image for tier ${tier}`, {
      size: data.length,
  ...
  clear(): void {
    ...
    this.cache.clear();
    BadgeLogger.debug(`Cleared image cache (removed ${size} entries)`);
  }
  ...
  clearExpired(): number {
    ...
    if (removed > 0) {
      BadgeLogger.debug(`Cleared ${removed} expired cache entries`);
    }
  ...
  private evictOldest(): void {
    ...
    if (oldestKey) {
      ...
      BadgeLogger.debug(`Evicted oldest cache entry (tier ${tier})`);
    }
  }
