...
const nextConfig: NextConfig = {
  ...
  // Public routes were renamed from game-pass → reservoir; keep old paths working (admin UI, bookmarks).
  async rewrites() {
    return [
      { source: '/api/game-pass/:path*', destination: '/api/reservoir/:path*' },
      { source: '/api/admin/game-pass/:path*', destination: '/api/admin/reservoir/:path*' },
    ];
  },
