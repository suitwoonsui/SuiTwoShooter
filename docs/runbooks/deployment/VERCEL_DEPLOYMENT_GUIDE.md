...
3. Configure the project:
   ...
   - **Build Command**: `cd ../.. && npm run build:frontend` (copies base config files)
   - **Output Directory**: `.` (serves from root directory - leave blank or set to `.`)
   - **Install Command**: `cd ../.. && npm install` (installs root dependencies for build script)
   
   **Important:** Make sure there's a **space** between `cd` and `../..` in the build command!
...
**Important Notes:**
- The build command copies base config files into the frontend directory (required for Vercel)
- After deploying the backend, update the backend URL via meta tag in `apps/shooter-game/frontend/index.html` or ensure the base config detects it correctly
- See `docs/codebase-cleanup/VERCEL_BUILD_SETUP.md` for detailed build process explanation
