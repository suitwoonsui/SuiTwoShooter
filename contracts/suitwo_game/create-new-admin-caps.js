  ...
  try {
    ...
    if (storeAdminResult.effects?.status?.status === 'success' && storeAdminResult.effects?.objectChanges) {
      for (const change of storeAdminResult.effects.objectChanges) {
        if (change.type === 'created' && change.objectType?.includes('AdminCapability') && change.objectType?.includes('premium_store')) {
    ...
    // 3. Tournament Admin Cap
    console.log('\n3️⃣ Creating Tournament Admin Capability...');
    ...
    tournamentAdminTx.setGasBudget(50_000_000);
    ...
    if (tournamentAdminCapId) {
      ...
      console.log('   Transaction:', tournamentAdminResult.digest);
    } else {
      console.warn('   ⚠️  Tournament Admin Cap not found');
      console.log('   Transaction:', tournamentAdminResult.digest);
      console.log('   Object Changes:', JSON.stringify(tournamentAdminResult.effects?.objectChanges, null, 2));
    }
